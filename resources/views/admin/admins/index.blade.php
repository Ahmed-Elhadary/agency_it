...
@section('breadcrumbs')
    ...
    <li class="breadcrumb-item active"><a href="#">{{__('lang.admins')}}</a></li>
@endsection
@section('title')
    {{__('lang.admins')}}
@endsection
@section('content')
    ...
    <div class="row">
        ...
        <div class="col-md-12">
            <div class="card">
                <div class="card-body">
                    <h4  style="font-weight: bold"> {{__('lang.admins')}}

                        <a onclick="addForm()" class="btn btn-primary pull-right text-white" style="float: left">
                            <span class="fa fa-plus-circle"></span> {{__('lang.add')}} {{__('lang.admins')}}
                        </a>
                    </h4>
                    <div class="col-lg-12 page-title-box">

                    </div>
                    <table id="admins-table" class="table table-bordered dt-responsive" style="border-collapse: collapse; border-spacing: 0; width: 100%;">
                        <thead>
                        <tr>
                            <th> {{__('lang.m')}}</th>
                            <th>  {{__('lang.name')}}</th>
...
@section('scripts')
    <script type="text/javascript">
        ...
        function addForm() {
            ...
            $('.modal-title').text('{{__("lang.add") .__("lang.admins")}}');
        }
        ...
        function editForm(id) {
            ...
            $.ajax({
                ...
                success: function(data) {
                    console.log(data);
                    $('#modal-form').modal('show');
                    $('.modal-title').text('{{__("lang.edit").__("lang.admins")}}');
                    $('#id').val(data.id);
                    $('#name').val(data.name);
